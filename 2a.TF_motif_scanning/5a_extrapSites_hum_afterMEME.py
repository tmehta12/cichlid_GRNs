...
if len(sys.argv) < 2:
    ...
    print('Usage: meme infile, JASPAR .sites dir\n')
    sys.exit()
    ...
    if len(motifin) >= 5:
        for a in pairwise2.align.localms(str(seqin), str(motifin), 2, -2, -10, -1):
            if 5 <= len(motifin) < 6:
                ...
                print('1mm -- score threshold == ' + str((len(motifin)*2)-4))
                print('\tmin length == ' + str((len(motifin)-1)))

                if a[4] - a[3] >= len(motifin):
                    if a[2] >= (len(motifin)*2) - 4:
                        print(format_alignment(*a))
                        # time.sleep(1)
                        if motifin not in motifout:
                            return motifin
                    ...
                    print('alignment too short @:\t' + str(a[4]-a[3]) + '\n')

            elif 6 <= len(motifin) < 12:
                print('2mm -- score threshold == ' + str((len(motifin)*2)-8))
                ...
                print('3mm --  score threshold == ' + str((len(motifin)*2)-12))
                ...
                print('\t4mm -- score threshold == ' + str((len(motifin)*2)-16))
...
# hocoParse = {}
# with open(sys.argv[7]) as hocomoco:
...
count_list        = []
# no_site_name_list = []
# with open(sys.argv[4]) as db_tftgtfbs_file, open(sys.argv[4].split('.')[0] + '.sites2.out', 'w') as out_file1:
...
#
#             print('Cic/data/motifs/extrapolation/hocomoco/human_words/' + hocoParse[line[0]] + '.words open...\n')
#             with open('Cic/data/motifs/extrapolation/hocomoco/human_words/' + hocoParse[line[0]] + '.words') as hoco_in:
#                 for word in hoco_in:
#                     print('\tTFBS: ' + line[2])
...
#
#                     try:
#                         os.mkdir(sys.argv[3] + 'tf_h2_' + line[0])
#                     except FileExistsError:
#                         print('Dir ' + sys.argv[3] + 'tf_h2_' + line[0] + ' already exists, writing there... ')
#
#
#                     if 'tf_h2_' + line[0] + '.fasta' in os.listdir(sys.argv[3] + 'tf_h2_' + line[0]):
#                         with open(sys.argv[3] + 'tf_h2_' + line[0] + '/tf_h2_' + line[0] +  '.fasta', 'a') as tf_out_fasta:
#                             tf_out_fasta.write('>' + str(counter).strip() + '_' + line[1].strip() + '\n')
#                             tf_out_fasta.write(line[2].strip() + '\n')
#                     else:
#                         with open(sys.argv[3] + 'tf_h2_' + line[0] + '/tf_h2_' + line[0] +  '.fasta', 'w') as tf_out_fasta:
#                             tf_out_fasta.write('>' + str(counter).strip() + '_' + line[1].strip() + '\n')
#                             tf_out_fasta.write(line[2].strip() + '\n')
#
#                     counter = counter + 1
...
#                 #############
#
# # print (no_site_name_list)
# # sys.exit()

print(sys.argv)

# print('Aligning using mafft...')
# subprocess.run(['mafft --localpair --maxiterate 1000 --quiet ' + sys.argv[3] + 'tf_h2_' + lol + '/tf_h2_' + lol + '.fasta > ' + sys.argv[3] + 'tf_h2_' + lol + '/tf_h2_' + lol + '_aln.fasta'], shell = True, check = True)
#
# print('Done.\n')
lol = sys.argv[1].strip().split('~')
print(lol)
# sys.exit()
# os.stat(sys.argv[3] + 'tf_h2_' + lol)



# print('Scanning for motifs using MEME...\n')
# # subprocess.run(['meme ' + sys.argv[3] + 'tf_h2_' + lol + '/tf_h2_' + lol + '.fasta -dna -oc ' + sys.argv[3] + 'tf_h2_' + lol + '/ -minw 5 -maxw 30 -maxsize 100000000 -nostatus'], shell = True, check = True)
#
# # print('sbatch -J memeTry -p ei-medium -t 5-0:00 -n 128 --mem=10000 -o Scr/logs/tf_h2_' + lol + '.STDOUT -e Scr/logs/tf_h2_' + lol + '.STDERR --wrap " \nsource lmod-6.1\nsource meme-4.10.1_4\nmpirun -np 100 meme Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_h2_' + lol + '/tf_h2_' + lol + '.fasta -dna -oc Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_h2_' + lol + '/ -p 128 -minw 5 -maxw 30 -maxsize 100000000 "')
# subprocess.run(['sbatch -J tf_m_' + lol + ' -p ei-medium -t 5-0:00 -n 128 --mem=20000 -o Scr/logs/tf_h2_' + lol + '.STDOUT -e Scr/logs/tf_h2_' + lol + '.STDERR --wrap " \nsource lmod-6.1\nsource meme-4.10.1_4\nmpirun -np 100 meme Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_h2_' + lol + '/tf_h2_' + lol + '.fasta -dna -oc Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_h2_' + lol + '/ -p 128 -minw 5 -maxw 30 -maxsize 100000000 "'], shell = True, check = True)

# print('Done.\n')
# sys.exit()
print()
print(str(sys.argv[2]) + 'tf_h2_' + lol[0] + '/meme.txt')

try:
    filer = open(sys.argv[2] + 'tf_h2_' + lol[0] + '/meme.txt')

    if os.stat(sys.argv[2] + 'tf_h2_' + lol[0]+'/meme.txt').st_size != 0:
        ...
        for stick, stack in store_dict.items():
            ...
            if '-' not in stack:
                if len(stack) == 5:
                    if stack[3] not in core_motif_list:
                        core_motif_list.append(stack[3])
                elif len(stack) == 3:
                        if stack[2] not in core_motif_list:
                            core_motif_list.append(stack[2])
                elif len(stack) == 4:
                    if len(stack[2]) > len(stack[3]):
                        if stack[2] not in core_motif_list:
                            core_motif_list.append(stack[2])
                    elif len(stack[3]) > len(stack[2]):
                        if stack[3] not in core_motif_list:
                            core_motif_list.append(stack[3])
                    else:
                        print('\n\n\n\tBreak due to motif indecision --- line 760\n\n\n')
                        sys.exit()
                else:
                    print('\n\n\n\tBreak due to no non-conventional motif --- line 763\n\n\n')
                    sys.exit()
            else:
                print('skipping due to gap in motif...')
        ...
        print('\n\nScript has found ' + str(len(longest_core)) + ' core motifs for ' + str(lol[0]) + '\n\n\n')

        with open(sys.argv[2] + 'tf_h2_' + lol[0] + '/tf_h2_' + lol[0] +  '.memeSites', 'w') as tf_out_fasta:
        ...
        with open(lol[1]) as db_tftgtfbs_file, open(lol[1].split('.')[0] + '.sites2.out', 'a') as out_file2:
            for line in csv.reader(db_tftgtfbs_file, delimiter = '\t'):
                line.append([])
                print('Target:\t' + str(line))
                print('Opening:\t' + sys.argv[2] + 'tf_h2_' + line[0] + '/tf_h2_' + line[0] + '.memeSites')
                try:
                    with open(sys.argv[2] + 'tf_h2_' + line[0] + '/tf_h2_' + line[0] + '.memeSites') as sites_file:
                        for site_id, site_seq in readFastaGenerator(sites_file):
                            print('\t\t' + site_id)
                            print('\t\t' + site_seq)
                            # time.sleep(.1)
