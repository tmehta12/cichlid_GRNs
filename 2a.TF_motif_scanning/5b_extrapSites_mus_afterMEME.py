...
#
# # print('sbatch -J memeTry -p ei-medium -t 5-0:00 -n 128 --mem=10000 -o Scr/logs/tf_m2_' + lol + '.STDOUT -e Scr/logs/tf_m2_' + lol + '.STDERR --wrap " \nsource lmod-6.1\nsource meme-4.10.1_4\nmpirun -np 100 meme Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_m2_' + lol + '/tf_m2_' + lol + '.fasta -dna -oc Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_m2_' + lol + '/ -p 128 -minw 5 -maxw 30 -maxsize 100000000 "')
# subprocess.run(['sbatch -J tf_m_' + lol + ' -p ei-medium -t 5-0:00 -n 128 --mem=20000 -o Scr/logs/tf_m2_' + lol + '.STDOUT -e Scr/logs/tf_m2_' + lol + '.STDERR --wrap " \nsource lmod-6.1\nsource meme-4.10.1_4\nmpirun -np 100 meme Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_m2_' + lol + '/tf_m2_' + lol + '.fasta -dna -oc Cic/data/motifs/extrapolation/JASPAR2018/sites/tf_m2_' + lol + '/ -p 128 -minw 5 -maxw 30 -maxsize 100000000 "'], shell = True, check = True)

# print('Done.\n')
# sys.exit()
print()
print(str(sys.argv[2]) + 'tf_m2_' + lol[0] + '/meme.txt')

try:
    filer = open(sys.argv[2] + 'tf_m2_' + lol[0] + '/meme.txt')

    if os.stat(sys.argv[2] + 'tf_m2_' + lol[0]+'/meme.txt').st_size != 0:
        ...
        with open(sys.argv[2] + 'tf_m2_' + lol[0] + '/tf_m2_' + lol[0] +  '.memeSites', 'w') as tf_out_fasta:
        ...
        with open(lol[1]) as db_tftgtfbs_file, open(lol[1].split('.')[0] + '.sites2.out', 'a') as out_file2:
            for line in csv.reader(db_tftgtfbs_file, delimiter = '\t'):
                line.append([])
                print('Target:\t' + str(line))
                print('Opening:\t' + sys.argv[2] + 'tf_m2_' + line[0] + '/tf_m2_' + line[0] + '.memeSites')
                try:
                    with open(sys.argv[2] + 'tf_m2_' + line[0] + '/tf_m2_' + line[0] + '.memeSites') as sites_file:
                        for site_id, site_seq in readFastaGenerator(sites_file):
                            print('\t\t' + site_id)
                            print('\t\t' + site_seq)
                            # time.sleep(.1)
