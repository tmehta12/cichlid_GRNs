...
subprocess.run(['mkdir matQualOutput_wg/mouse'], shell = True, check = True)
            ...
            for lul in sorted(os.listdir(os.path.join(sys.argv[1], lol, lel, lal))):
                if os.path.isdir(os.path.join(sys.argv[1], lol, lel, lal, lul)):
                    ...
                    for lil in sorted(os.listdir(os.path.join(sys.argv[1], lol, lel, lal, lul))):
                        ...
                        if lil == 'fimo.txt':
                            with open(os.path.join(sys.argv[1], lol, lel, lal, lul, lil)) as data_file, open('matQualOutput_wg/mouse/' + '_'.join([lal.split('_')[1], lul, lel, 'results.out']), 'a') as outie:
