            ...
            for lol in shop[1]:
                ...
                if isinstance(lol, list):
                    for ip in lol[1:7] + lol[8:]:
                        ...
                        if ip[0] != 'E':
                            if ip != 'NA':
                                # print ('RG-cich/CSPSSMs/mm10_' +  ip + ' horn')
                                cspssm_results_list.append(getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  ip))
                                if ip[0] != 'E':
                                    if ip[0] != 'N':
                    ...
                    if lol.startswith('O'):
                        ...
                        # print('HI')
                        # print ('RG-cich/CSPSSMs/mm10_' +  lol + ' dorn')
                        cspssm_results_list.append(getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  lol))
                        if lol[0] != 'E':
                            if lol[0] != 'N':
...
with open('mm10_mat_qual_pvals.out1') as jas_cspssm_dat, open('mm10_mat_qual_pvals_noJAS.out1', 'w') as out_file2:
    ...
    for count, (og, line) in enumerate(sorted(ortho_reclaim_store.items(), key = lambda x: int(x[0].split('_')[0].replace('OG', ''))), start = st_c + 1):
        # print(og + '\t' + ';'.join(['|'.join(str(i) for i in getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x)) if getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x) != 'NA' else getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x) for x in line[2]]))
        out_file2.write('motif_' + str(count) + '\tNA;' + str(line[1]) + ';' + '\t'.join([line[0], og, ';'.join(line[2]) , '0.0001' , ';'.join(['NA', 'NA', 'NA', 'NA', 'NA']) , ';'.join(['|'.join(str(i) for i in getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x)) if getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x) != 'NA' else getCSPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/mm10_' +  x) for x in line[2]])]) + '\n')
...
def addInCWPSSMvals (resultsMatrix):
    ...
    with open(resultsMatrix) as results_to_add_to:
        for line in csv.reader(results_to_add_to, delimiter = '\t'):
            # print(line)
            # print('RG-cich/CSPSSMs/' + line[1].split(';')[2].replace('ENSMUSG000000', 'mE_'))
            result     = getCWPSSMsMatrixQualityResultsVals('RG-cich/CSPSSMs/' + line[1].split(';')[2].replace('ENSMUSG000000', 'mE_'))
            new_result = []
