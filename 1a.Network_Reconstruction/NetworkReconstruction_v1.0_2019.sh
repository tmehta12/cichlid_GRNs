...
# Run script: ./NetworkReconstruction_v1.0_2019.sh [workingdir] Module_genesandexpr TFBSs TFTGco Edge_Attributes # NOTE - near the end there are instances of using nano, so do not run all in one go
# Files in [workingdir]:
...
awk '{if($2 == "NONE")print $1, $2, "NONE";else print $1, $2, $3;}' OFS='\t' $geneNamesMapping > geneNamesMapping2.txt
